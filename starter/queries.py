...
############################# TO DO #####################################
    ...
    top_products_query = """ SELECT product_category, SUM(unit_price*quantity)
                        FROM invoices
...
############################# TO DO ####################################
  ...
  #### create garden_output variable with formatted garden_query
    ...
    top_products_output = "COPY ({0}) TO STDOUT WITH CSV HEADER".format(top_products_query)
...
############## CREATING CSV FILES FROM OUTPUTS #########################
    ...
    with open('product_revenue.csv', 'w') as file:
        cursor.copy_expert(top_products_output, file)
